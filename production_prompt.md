...
Perform the following checks. Note that you will run as root so be careful not to remove anything We just need the report:
...
2.6 Check that Nginx rate-limiting is applied on authentication endpoints (e.g., /accounts/login/)
...
6.1 /srv/harada/.env → 600 (owner: deploy/app user only)
6.2 /srv/harada/ → not owned by or readable by www-data beyond what's necessary
6.3 Gunicorn socket file → correct group permissions for Nginx to read
